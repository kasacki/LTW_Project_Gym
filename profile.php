...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submit_class_review'])) {
        $memberId = (int)($userData['member_id'] ?? 0);
        $classId = (int)($_POST['class_id'] ?? 0);
        $rating = (int)($_POST['rating'] ?? 0);
        $comment = trim($_POST['comment'] ?? '');

        if ($_SESSION['role'] !== 'member' || !$memberId) {
            $error = "Only members can review classes.";
        } elseif ($classId <= 0 || $rating < 1 || $rating > 5) {
            $error = "Please select a valid class and rating.";
        } else {
            $stmt = $pdo->prepare("
                SELECT c.id
                FROM enrollments e
            ...
            if (!$stmt->fetch()) {
                ...
                try {
                    $stmt = $pdo->prepare("
                        INSERT INTO reviews (member_id, class_id, rating, comment)
                        VALUES (?, ?, ?, ?)
                        ON CONFLICT(member_id, class_id)
                        DO UPDATE SET rating = excluded.rating,
                                      comment = excluded.comment,
                                      created_at = CURRENT_TIMESTAMP
                    ");
                    $stmt->execute([$memberId, $classId, $rating, $comment]);
                    $message = "Class review saved.";
                } catch (Exception $e) {
                    $error = "Error while saving review: " . $e->getMessage();
                }
            }
        }
    }
...
if ($memberId) {
    $stmt = $pdo->prepare("
        SELECT c.id, c.name, c.type, c.scheduled_at, c.duration_min,
        ...
        LEFT JOIN reviews r ON r.class_id = c.id AND r.member_id = e.member_id
        WHERE e.member_id = ?
